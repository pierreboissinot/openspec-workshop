...
## Purpose
Permettre à l'utilisateur d'attribuer un niveau d'importance (`high`, `medium`,
`low`, défaut `medium`) à ses tâches, de le modifier, et de retrouver en premier
ce qui compte : `listTasks()` trie par importance décroissante en conservant
l'ordre d'ajout à priorité égale (tri stable). Les valeurs invalides sont
rejetées sans modifier l'état.
